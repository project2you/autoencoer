...
# Enhanced Memory Module with improved validation
class Memory(nn.Module):
    def __init__(self, memory_size, feature_dim, key_dim, temp_update, temp_gather):
        ...
        self.memory_age = torch.zeros(memory_size)
    ...
    def get_update_query(self, mem, max_indices, update_indices, score, query, train):
        ...
        for i in range(m):
            ...
            a, _ = idx.size() if len(idx.size()) > 1 else (idx.size(0), 1)
    ...
    def get_score(self, mem, query):
        ...
        score_query = F.softmax(score / self.temp_gather, dim=0)
        score_memory = F.softmax(score / self.temp_update, dim=1)
...
# Backward compatibility - เพิ่ม MemoryAdvanced เป็น alias ของ Memory
...
# Backward compatibility - เพิ่ม convAEAdvanced เป็น alias ของ convAE
...
def test_model_enhanced(args):
    ...
    feature_distances_raw = []  # Track raw feature distances
    ...
    with torch.no_grad():
        ...
        for k, (imgs) in pbar:
            ...
            # Memory state before forward pass
            ...
            # 🔍 Track memory updates and feature distances
    ...
    if memory_usage_history:
        ...
        # Check if memory is actually being used
    ...
    for video in tqdm(sorted(videos_list), desc="Processing videos"):
        ...
        # Calculate anomaly scores for this video
    ...
    memory_analysis = {
        ...
        'memory_usage_history': memory_usage_history[:100]  # Save first 100 steps
    }
...
# Model Manager Class for organized model handling
